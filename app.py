...
def read_conversion_factors():
    ...
    with open("factores_conversion.csv", newline='', encoding="latin-1") as csvfile:
        ...
        # Leer la cabecera; se asume que el primer elemento es una etiqueta
        header = next(reader)
        # Convertir los encabezados (a partir del segundo elemento) a enteros, eliminando espacios
        target_years = [int(col.strip()) for col in header[1:]]
        # Iterar sobre cada fila del archivo
        for row in reader:
            if not row:
                continue
            # Convertir el primer elemento a entero (año base)
            try:
            ...
            factors_for_base = {}
            # Iterar sobre las celdas restantes (usando enumerate para obtener el índice)
...
def main():
    ...
    # Ejemplo de cómo leer los factores de conversión
    # Los factores se pueden usar en procesos de conversión en otras partes de la aplicación
    conversion_factors = read_conversion_factors()
    st.sidebar.markdown("**Factores de Conversión cargados**")
    st.sidebar.write(conversion_factors)
